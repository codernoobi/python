# In Machine Learning, classification is a supervised learning approach, which can be thought of as a means of categorizing or "classifying" 
...
# If previous loan default data can be used to predict which customers are likely to have problems repaying loans, these "bad risk" 
# customers can either have their loan application declined or offered alternative products.
# The goal of a loan default predictor is to use existing loan default data, which is information about the customers (such as age, 
# income, education, etc.), to build a classifier, pass a new customer or potential future defaulter to the model, and then label it 
# (i.e. the data points) as "Defaulter" or "Not Defaulter", or for example, 0 or 1.
...
# For Churn detection, where we predict whether a customer switches to another provider or brand; Or to predict whether or not a 
# customer responds to a particular advertising campaign.
...
# For example, classification can be used for email filtering, speech recognition, handwriting recognition, bio-metric identification, 
